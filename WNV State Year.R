...
latlong.pred.yr = function(year){
  ...
  pred_qp = predict(species.zigam, sp.pred.year, type="response", exclude="s(locID)")
  pred_qp = data.frame(cbind(x = grid@coords[,"x1"], y = grid@coords[,"x2"], z = pred_qp))
  return(pred_qp)
}
...
for(i in 1:frames){
  ...
  if (i < 10) {name = paste("~/Desktop/WNV_visuals/SpatialPredictionsTesting/000",i,"plot.png",sep="")}
  if (i < 100 && i >= 10) {name = paste("~/Desktop/WNV_visuals/SpatialPredictionsTesting/00",i,"plot.png", sep="")}
  if (i >= 100) {name = paste("~/Desktop/WNV_visuals/SpatialPredictionsTesting/0", i,"plot.png", sep="")}
  ...
  plot(nj.yr.raster[[i]], col=colorRampPalette(c("red", "yellow"))(7), zlim=c(0.5,3.5),
       main=paste0("Crow Abundance, ", as.character(1995+i)), axes=F, frame.plot=F, box=F)
