...
#----Setup----
if(T){
  setwd("C:/Users/itloaner/Desktop/WNV2")
}

#----Importing PFW data----
if(T){
  raw.pfw = read.csv("PFW_amecro_zerofill.csv")
...
#----Reassigning land classes to pfw data----
...
if(T){
  nnn.pred <- length(unique(pfwct$yr)) #This is the variable you want to predict
  xxx <- seq(from=1996,
             to = 2017,
             length = nnn.pred)
  ...
  pred.test<- stats::predict(
    species.zigam,
    newdata = traj.data, type="response", interval= "confidence",
    se.fit= TRUE, exclude="s(locID)") # This now excludes re:locID from the prediction!
...
if(T){
  ...
  text(1999,min(plot_data$Count),"WNV Introduction", col="gray", cex=0.7, pos=4 ,srt=90, offset=0.5)
  ...
  lines(pred.test$fit-1.96*pred.test$se.fit ~ xxx,lwd=1, col="gray40", lty=1)
  text(1999,min(plot_data$Count),"WNV Introduction", col="gray",
       cex=0.7, pos=4,srt=90, offset=0.5)
}

#----Percent Decline Plot----
if(T){
  plot.new()
  
  error_top = pred.test$fit+1.96*pred.test$se.fit
  error_bottom = pred.test$fit-1.96*pred.test$se.fit
  
  ymax = max(error_top) + (0.15*(max(error_top)-min(error_bottom)))
  ymin = min(error_bottom) - (0.75*(max(error_top)-min(error_bottom)))
  
  lambda = function(y){
    return(c(0, 1*y[4]/4, 2*y[4]/4, 3*y[4]/4, 4*y[4]/4, 5*y[4]/4))
  }
  
  par(mar=c(5, 4, 2, 5))
  plot(NULL, pch=20, col="black", 
       frame.plot=T, main=paste0("American Crow - ", "NJ"),
       xlab = "Year", ylab = "λ (Relative to 1999)", lwd=2.5,
       ylim=c(ymin, ymax), axes=F, xlim=c(1996,2017))
  ...
  lines(plot_data$Count~plot_data$Year,lwd=2, col="black")
  lines(error_top ~ plot_data$Year,lwd=1, col="gray40", lty=1)
  lines(error_bottom ~ plot_data$Year,lwd=1, col="gray40", lty=1)
  abline(v = 1999, lty=2, lwd = 2, col = "gray35")
  text(1999,min(plot_data$Count),"WNV Introduction", cex=0.7, pos=4 ,srt=90, offset=0.5)
  abline(h=lambda(plot_data$Count), col="black", lwd=0.2, lty=1)
  axis(side=2, at=lambda(plot_data$Count), labels=c("-100%", "-75%", "-50%", "-25%", "0", "+25%"))
  axis(side=1, at=seq(2000,2015, by=5), labels=seq(2000,2015, by=5)) # Adding x-axis
}